...
def piecewise_factor(t: float, ts: List[float], ys: List[float]) -> float:
    if not ts or not ys or len(ts) != len(ys):
        return 1.0
    return lin_interp(t, ts, ys)
...
def air_density_moist(P_Pa: float, T_K: float, RH: float) -> float:
    """Densidad aire húmedo (kg/m3)."""
    e = clamp(RH, 0.0, 1.0) * sat_vapor_pressure_water(T_K - 273.15)
    Pd = P_Pa - e
    return Pd / (R_DRY * T_K) + e / (R_WV * T_K)

def mu_air_sutherland(T_K: float) -> float:
    """Viscosidad dinámica del aire (Pa·s) Sutherland."""
    T0 = 273.15; mu0 = 1.716e-5; C = 111.0
...
@dataclass
class FilterConfig:
    dp0_curve: Dict[str, List[float]]
    alpha_Pa_per_kg: float = 0.0
    eta_filter: float = 0.9
    dp_max_Pa: float = 0.0
    enabled: bool = True
    cleaning_interval_s: float = 0.0
    cleaning_drop_frac: float = 0.0
...
class FanConfig:
    ...
    eta_fan: float = 0.6
    eta_motor: float = 0.9
    vfd_times_s: List[float] = field(default_factory=list)
    vfd_N: List[float] = field(default_factory=list)

    def dp(self, Q_m3_s: float) -> float:
        return lin_interp(Q_m3_s, self.curve["Q"], self.curve["DP"])

    def dp_scaled(self, Q_m3_s: float, N: float) -> float:
        """Afinidad: dp_N(Q) ≈ N^2 * dp_base(Q/N)."""
        Qeff = Q_m3_s / max(N, 1e-9)
        base = lin_interp(Qeff, self.curve["Q"], self.curve["DP"])
        return (N * N) * base
...
class Species:
    ...
    x_liq: float
    LFL_vol_frac: Optional[float] = None
...
class Simulator:
    ...
    def system_dp_without_fan(self, Q: float) -> Tuple[float, float, float, List[Dict[str, float]]]:
        ...
        if Ke and Ke > 0 and self.cfg.ducts:
            D0 = self.cfg.ducts[0].D
            A0 = math.pi * D0 * D0 / 4
            v0 = Q / A0 if A0 > 0 else 0.0
            total_dp += 0.5 * self.rho_air * v0 * v0 * Ke
        total_dp += self.filter_dp(Q)
        v0_out = v_re_list[0]["v"] if v_re_list else 0.0
        Re0_out = v_re_list[0]["Re"] if v_re_list else 0.0
        return total_dp, v0_out, Re0_out, v_re_list

    def find_operating_Q(self) -> Tuple[float, Dict[str, float], List[Dict[str, float]]]:
        fan = self.cfg.fan
        Q_floor = max(0.0, self.cfg.sim.Q_floor_m3_s or 0.0)
        Qmin_curve = max(0.0, min(fan.curve["Q"]))
        Qmax_curve = max(fan.curve["Q"])
        Qlo, Qhi = max(0.0, Q_floor), Qmax_curve * 1.2

        N = piecewise_factor(self.state["t"], fan.vfd_times_s, fan.vfd_N)

        def F(Q: float) -> float:
            sys_dp, _, _, _ = self.system_dp_without_fan(Q)
            return fan.dp_scaled(Q, N) - sys_dp
        ...
        if f_lo * f_hi <= 0:
            ...
            return Q, {"DP_fan_Pa": fan.dp_scaled(Q, N), "DP_sys_Pa": sys_dp,
        ...
        if f_lo < 0 and f_hi < 0:
            Q = max(Q_floor, Qmin_curve)
        elif f_lo > 0 and f_hi > 0:
            Q = max(Q_floor, Qmax_curve)
        else:
            Q = max(Q_floor, best_Q)
    ...
    def species_equilibrium_gas(self) -> Dict[str, float]:
        ...
        for sp in self.cfg.mixture.species:
            A, B, C = sp.Antoine
            Psat_bar = max(0.0, antoine_P_bar(T, A, B, C))
        ...
        for i in names:
            ...
            for j in names + ["AIR"]:
                ...
                if Dij <= 0:
                    Dij = 1e-12
                denom += yj / Dij
            out[i] = 1.0 / denom if denom > 0 else 1.0
        return out

    def mass_transfer_coeffs(self, Q: float) -> Dict[str, float]:
        geom = self.cfg.geometry
        u = Q / (geom.area_face if geom.area_face > 0 else 1.0)
        nu = self.mu_air / self.rho_air
    ...
    def step(self, dt: float) -> Dict[str, float | bool]:
        ...
        if cfg.sim.Q_fixed_m3_s and cfg.sim.Q_fixed_m3_s > 0:
            Q_raw = cfg.sim.Q_fixed_m3_s
            sys_dp, v0, Re0, v_re_list = self.system_dp_without_fan(Q_raw)
            diag_fan = {"DP_fan_Pa": self.cfg.fan.dp(Q_raw), "DP_sys_Pa": sys_dp,
                        "v_duct0_m_s": v0, "Re_duct0": Re0}
        else:
            Q_raw, diag_fan, v_re_list = self.find_operating_Q()

        Q_floor = max(0.0, cfg.sim.Q_floor_m3_s or 0.0)
        Q = max(Q_raw, Q_floor)

        if Q != Q_raw:
            sys_dp, v0, Re0, v_re_list = self.system_dp_without_fan(Q)
            diag_fan["DP_sys_Pa"] = sys_dp
            diag_fan["v_duct0_m_s"] = v0
            diag_fan["Re_duct0"] = Re0
        ...
        # Captura filtro
        eta_f = clamp(cfg.filter.eta_filter if cfg.filter.enabled else 0.0, 0.0, 1.0)
        captured_kg_s = eta_f * Q * (C_p_next / 1e6)
        ...
        # Limpieza de filtro (si aplica)
        ci = cfg.filter.cleaning_interval_s
        drop = cfg.filter.cleaning_drop_frac
        if cfg.filter.enabled and ci > 0 and drop > 0:
            if (self.state["t"] - self._t_last_clean) >= ci:
                self.state["M_captured_kg"] *= (1.0 - drop)
                self._t_last_clean = self.state["t"]

        # diagnósticos especies
        y_eq = self.species_equilibrium_gas()
        names = [sp.name for sp in cfg.mixture.species]
        Mavg = 0.0
        for nm, sp in zip(names, cfg.mixture.species):
            Mavg += y_eq.get(nm, 0.0) * sp.M_g_mol
        if Mavg <= 0: Mavg = 60.0
        n_tot = self.P_Pa / (R_UNIVERSAL * self.T_K)
        y_voc = ((C_v_next / 1000.0) / Mavg) / n_tot
        ...
        if tot_y_solvs > 0:
            for sp in cfg.mixture.species:
                C_i[sp.name] = (y_eq.get(sp.name, 0.0) / tot_y_solvs) * C_v_next
        else:
            for sp in cfg.mixture.species:
                C_i[sp.name] = 0.0
        ...
        for sp in cfg.mixture.species:
            ...
            J0[sp.name] = h * C_i[sp.name]

        dp_sys = diag_fan.get("DP_sys_Pa", 0.0)
        P_shaft = Q * dp_sys / max(cfg.fan.eta_fan, 1e-6)
        P_elec = P_shaft / max(cfg.fan.eta_motor, 1e-6)

        v_duct0 = diag_fan.get("v_duct0_m_s", None)
        if (v_duct0 is None) and self.cfg.ducts:
            D0 = self.cfg.ducts[0].D
            A0 = math.pi * D0 * D0 / 4.0
            v_duct0 = Q / A0 if A0 > 0 else 0.0

        out: Dict[str, float | bool] = {
            "t_s": self.state["t"],
            "Q_m3_s": Q,
            "V_face_m_s": u_face,
            "v_duct_m_s": v_duct0 or 0.0,
            "DP_system_Pa": dp_sys,
        ...
        if cfg.criteria.v_duct_obj is not None:
            out["ok_v_duct"] = ((v_duct0 or 0.0) >= cfg.criteria.v_duct_obj)
    ...
    def run(self, dt: Optional[float] = None, t_end: Optional[float] = None) -> List[Dict[str, float | bool]]:
...
def config_from_dict(d: Dict) -> SystemConfig:
    ...
    filter_cfg = FilterConfig(
        ...
        cleaning_interval_s=filt.get("cleaning_interval_s", 0.0),
        cleaning_drop_frac=filt.get("cleaning_drop_frac", 0.0),
    )
    ...
    fan_cfg = FanConfig(
        ...
        vfd_times_s=fan.get("vfd_times_s", []),
        vfd_N=fan.get("vfd_N", []),
    )
    ...
    sim = SimSetup(
        ...
        Q_floor_m3_s=sm.get("Q_floor_m3_s", 0.0),
    )
